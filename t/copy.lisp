  ...
  (with-foreign-objects ((devices 'cl-device-id)
    ...
    (let ((platform (get-platform-id)))
      ...
      (let ((context (create-context (null-pointer) 1 devices))
        ...
        (with-buffer (in context +cl-mem-read-only+ 1)
          (ok in "create buffer")
          (with-buffer (out context +cl-mem-write-only+ 1)
  ...
  (with-foreign-objects ((devices 'cl-device-id)
    ...
    (let ((platform (get-platform-id)))
      ...
      (let ((context (create-context (null-pointer) 1 devices))
            (device (mem-aref devices 'cl-device-id)))
        (ok context "create context")
        (with-buffer (buffer-in context +cl-mem-read-only+ 1)
          (with-buffer (buffer-out context +cl-mem-write-only+ 1)
